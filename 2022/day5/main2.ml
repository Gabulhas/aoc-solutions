...
let execute_moves rear =
    ...
    in
    let execute_move amount source destination rear =
        let rec pop_multiple amm r all_popped=
            if amm > 0 then
                let (popped, new_rear) = pop_rearrarrangement_pos source r in
                pop_multiple (amm - 1) new_rear (popped::all_popped)
            else
                (r, all_popped)

        in
        let (new_rear, all_popped) = pop_multiple amount rear [] in
        List.fold_left (fun r popped -> push_rearrarrangement_pos popped destination r) new_rear all_popped
